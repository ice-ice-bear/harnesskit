...
| **Insights 기반 skill 자동 생성** | 반복 패턴 감지 → 새 skill 파일 자동 생성 → "이 에러 핸들링 패턴을 skill로 추출할까요?" | v1 failures + session-logs 분석 |
...
| **A/B 테스트** | `/skill-builder` 연동으로 skill/hook 변형 테스트 | skill-builder 플러그인 |
...
### 9.2 [A] Repo 맞춤 Skills 자동 생성

감지된 프레임워크에 따라 `.claude/skills/` (또는 프로젝트 내 `harnesskit-skills/`)에 skill 파일을 생성한다. 템플릿 기반이므로 토큰 소비 없음.
...
insights는 harness 인프라뿐 아니라 **toolkit도 개선 대상**이다:

| 대상 | 제안 유형 | 예시 |
|------|----------|------|
| **Skills** | 내용 수정/추가 | "nextjs-conventions.md에 Image 최적화 규칙 추가 권장" |
| **Skills** | 새 skill 참조 추가 | "에러 핸들링 패턴이 반복됩니다, error-handling.md skill 추가 권장" |
| **Dev Hooks** | 활성화/비활성화 | "typecheck hook이 매번 실패 후 무시됩니다, 비활성화할까요?" |
| **Dev Hooks** | 설정 조정 | "lint hook이 너무 느립니다, --cache 옵션 추가 권장" |
| **Dev Commands** | 옵션 조정 | "/harnesskit:test에 --watch 모드 추가 권장" |
| **Marketplace** | 추가 추천 | "코드 리뷰를 자주 요청합니다, /review 플러그인 설치 권장" |
| **Agents** | 추가 추천 | "API 조사에 시간이 많이 걸립니다, researcher agent 설치 권장" |
