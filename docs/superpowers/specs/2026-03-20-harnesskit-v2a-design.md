...
③ proposal에 스크립트 내용을 diff로 표시
④ 승인 시:
   ├─ .harnesskit/hooks/{name}.sh에 저장
   ├─ chmod +x
   ├─ .claude/settings.json의 적절한 hook 지점에 등록 (PreToolUse/PostToolUse)
   └─ config.json의 customHooks에 기록
...
Stage 3: Replace (review_replace 승인)
  └─ marketplace /review 제거
  └─ .harnesskit/skills/code-review.md가 전체 리뷰 담당
  └─ insights가 계속 모니터링: 품질 저하 시 → marketplace 재설치 제안
```
...
| 필드 | 용도 |
|------|------|
| `uncoveredAreas` | setup 시 marketplace에서 매칭 plugin을 찾지 못한 영역. insights의 skill_creation 트리거 입력 |
...
| 신규 필드 | 용도 | 생성 주체 |
|----------|------|----------|
| `toolCallSequences` | 반복 tool call 패턴 감지 → hook_creation 트리거 | session-end.sh |
| `taskTimeDistribution` | 작업 유형별 시간 분포 → agent_creation 트리거 | session-end.sh |
| `pluginUsage` | marketplace plugin 사용량 + 피드백 → review 내재화 + plugin 효과 추적 | session-end.sh |

### 5.3 session-end.sh — 신규 책임

v1의 기존 책임 (로그 저장 + failures 기록 + 넛지 감지)에 추가:

| 책임 | 구현 | 토큰 소비 |
|------|------|----------|
| Tool call 시퀀스 감지 | `current-session.jsonl` 스캔 → 3+번 반복 패턴 추출 | 0 |
| 시간 분포 추정 | tool call 유형별 분류 (Bash:test/lint = debugging, WebSearch = research, Edit/Write = coding) | 0 |
| Plugin 사용량 추출 | 세션 중 marketplace plugin 호출 스캔 | 0 |

모두 shell 기반, 토큰 0.

### 5.4 current-session.jsonl — 확장 이벤트

v1의 이벤트 (error, feature_done, feature_fail)에 추가:

```jsonl
{"type":"plugin_invocation","plugin":"review","feedback":["missing-error-boundary"]}
{"type":"tool_sequence","sequence":["Bash:tsc","Edit:fix","Bash:tsc"],"count":3}
```

CLAUDE.md의 세션 프로토콜에 다음 규칙을 추가:
- marketplace plugin 사용 시: `current-session.jsonl`에 plugin_invocation 기록
- 리뷰 피드백 받을 시: feedbackThemes에 핵심 키워드 기록
...
- `session-start.sh`가 `schemaVersion` 불일치 감지 → `/harnesskit:setup` 마이그레이션 안내
- 마이그레이션: `config.json`에 누락 필드 추가 (비파괴적)
- 기존 session-logs는 그대로 동작 (누락 필드는 insights가 null로 처리)
