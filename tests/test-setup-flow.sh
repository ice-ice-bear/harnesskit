...
check "Has name field value" "jq -e '.name' '$manifest' >/dev/null"

echo ""
echo "=== Test: Skill and agent files exist ==="
check "setup SKILL.md exists" "[ -f '$SCRIPT_DIR/../skills/setup/SKILL.md' ]"
check "orchestrator AGENT.md exists" "[ -f '$SCRIPT_DIR/../agents/orchestrator/AGENT.md' ]"
